...
export class ToastrService {
  ...
  async danger(message: string, header: string = 'Error'): Promise<void> {
    const icon = 'alert-circle-outline';
    const color = 'danger';
    await this.presentToast({ message, color, header, icon });
  }

  async success(message: string, header: string): Promise<void> {
    const icon = 'checkmark-circle-outline';
    const color = 'success';
    await this.presentToast({ message, color, header, icon });
  }

  async warning(message: string, header: string): Promise<void> {
    const icon = 'warning-outline';
    const color = 'warning';
    await this.presentToast({ message, color, header, icon });
  }

  async info(message: string, header: string): Promise<void> {
